...
<div class="w-full max-w-4xl">
  <div style="display: flex; flex-wrap: wrap;">
    <div v-for="rowIndex in gridSize" :key="rowIndex" class="flex w-full">
      <div
      ...
      :class="{
        'bg-blue-100':
        selectedToken !== null &&
        selectedToken.index === (rowIndex - 1) * gridSize + (colIndex - 1),
      }"
      >
      <div
      v-if="tokens.some(
      (token) =>
      token.index === (rowIndex - 1) * gridSize + (colIndex - 1)
      )"
      :class="{
        'bg-green-500':
        selectedToken !== null &&
        ...
        selectedToken === null ||
        selectedToken.index !==
        (rowIndex - 1) * gridSize + (colIndex - 1),
      }"
